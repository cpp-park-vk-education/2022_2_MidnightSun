...
void CurrentPlaylistUIModel::deleteTrack(size_t index) {
    bool flag = false;
    ...
    if (flag) {
        mediator_->notifyTrackPlaylist(this, DeletedTrackIsCurrent, playlist_.currentTrack());
    }
}

void CurrentPlaylistUIModel::chooseTrack(size_t index) {
    playlist_.chooseTrack(index);
    mediator_->notifyTrackPlaylist(this, NewCurrentTrack, playlist_.currentTrack());
}
